...
const char* mqtt_server = "192.168.43.219"; // Example Server IP
...
// MQTT v2 Callback Signature (topic, payload, length)
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "smartbin/BIN-001/command") {
    ...
    if (String(cmd) == "START_BIN") {
      ...
      // Logic to trigger relay/servo for door
      delay(5000); // simulate user throwing trash
...
void setup() {
  ...
  esp_sleep_enable_timer_wakeup(30 * 60 * 1000000ULL); // 30 mins
...
void loop() {
  ...
  // If not connected, BLE will be active to allow offline sync until sleep
}
