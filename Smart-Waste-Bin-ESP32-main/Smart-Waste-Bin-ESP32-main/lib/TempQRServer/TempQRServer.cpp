...
void TempQRServer::init() {
    ...
    server.on("/", [this]() {
        ...
        html += "    if(d.isBusy) {";
        html += "      root.innerHTML = `<h2 class='status-busy'>⚠️ Bin is Busy</h2>` + ";
        html += "                       `<h3>Time Remaining: ${d.timer} seconds</h3>` + ";
        html += "                       `<hr><p>Last Waste: <b>${d.lastType}</b></p>` + ";
        html += "                       `<p>Plastic: ${d.pLvl}%</p><div class='level-bar'><div class='fill' style='width:${d.pLvl}%'></div></div>` + ";
        html += "                       `<p>Metal: ${d.mLvl}%</p><div class='level-bar'><div class='fill' style='width:${d.mLvl}%'></div></div>`;";
        html += "    } else {";
        html += "      root.innerHTML = `<h3>Scan QR Code to Start</h3><div id='qr'>${d.qr}</div><p>Session Code: ${d.token}</p>`;";
        html += "    }";
        html += "  });";
        html += "}";
        html += "setInterval(refresh, 1000);";
        html += "</script></head><body><div class='container' id='content'>Loading...</div></body></html>";
